...
class tbk
{
	 ...
	 * @return @param data->commission:佣金
	 */
	public function getItemDetailNew($num_iid,$platform='2',$ip='',$pid='',$relationId ='')
	{
	    if($pid=='') {
	        //没有的情况下使用默认推广位
	        $pid=$this->pid;
	    ...
	    if($res_info['code']==0) {
	        //商品折扣价格，保留2位小数，四舍五不入
	        $res_info['data']['zk_final_price']=keepTwoDecimalPlaces($res_info['data']['zk_final_price']);
	        //获取商品详情内容
	        ...
	        if($res_hdk['code']==1){
	            ...
	            $hgMsg=$res_hdk['data'];
	            //宝贝短标题
	            $res_info['data']['itemshorttitle']=$hgMsg['itemshorttitle'];
	            //宝贝推荐语
	            $res_info['data']['item_description']=$hgMsg['itemdesc'];
	            //优惠券总量
	            $res_info['data']['coupon_total_count']=$hgMsg['couponnum'];
	            //优惠券剩余量
	            $res_info['data']['coupon_remain_count']=$hgMsg['couponsurplus'];
	            //优惠券开始时间
	            $res_info['data']['coupon_start_time']=$hgMsg['couponstarttime'];
	            //优惠券结束时间
	            $res_info['data']['coupon_end_time']=$hgMsg['couponendtime'];
	            //优惠券信息
	            $res_info['data']['coupon_info']=$hgMsg['couponexplain'];
	            //优惠券面额
	            $res_info['data']['coupon_amount']=$hgMsg['couponmoney'];
	            //优惠券链接
	            $res_info['data']['coupon_click_url']=$hgMsg['couponurl'];
	            
	            //佣金比率(%)
	            $res_info['data']['commission_rate']=$hgMsg['tkrates'];
	            //佣金
	            $res_info['data']['commission']=($res_info['data']['zk_final_price']-$res_info['data']['coupon_amount'])*$res_info['data']['commission_rate']/100;
	            //保留2位小数，四舍五不入
	            $res_info['data']['commission']=keepTwoDecimalPlaces($res_info['data']['commission']);
	            $res=$res_info;
	        }else {
	            $res=$res_hdk;
	        }
	...
	public function getItemDetail($num_iid,$platform='2',$ip='',$pid='',$relationId ='')
	{
		if($pid=='') {
			//没有的情况下使用默认推广位
			$pid=$this->pid;
	 ...
	 * @return @param data->num_iid:商品ID
	 */
	public function getTqgJu($pid,$adzone_id,$start_time,$end_time,$page_no=1,$page_size=40)
	{
		$c = new TopClient;
		$c->appkey = $this->appkey;
		$c->secretKey = $this->secret;
		...
		$req->setAdzoneId($adzone_id);
		$req->setFields("title,total_amount,click_url,category_name,zk_final_price,end_time,sold_num,start_time,reserve_price,pic_url,num_iid");
		...
		if($result['code']) {
			...
			for($i=0;$i<$num;$i++) {
				...
				//佣金
				$goodslist[$i]['commission']=($goodslist[$i]['zk_final_price']-$goodslist[$i]['coupon_amount'])*$goodslist[$i]['commission_rate']/100;
				//保留2位小数，四舍五不入
